...
int main(void) {
    ...
    decltype(i) number02 = 23;  // number02=const int ，i是什么类型，number02就是什么类型
    ...
    // 2.decltype后的圆括号是个表达式，不是个变量。返回表达式结果对应类型
    decltype(34) number08 = 5;  // number08=5;

    int j = 0;
    int *p1 = &j;
    int &reference02 = j;
    decltype(reference02 + 1) number09 = 454;  // number09=int 
    decltype(p1) p2 = &j;                      // p2=int*类型
    *p2 = 43;
    decltype(*p2) number10 = j; //number10=int&类型
    /*
     * *p2是指针p2所指向的内存空间，而且能够给内存空间赋值，所以是个左值
 ...
 * decltype对于给定的变量名或者表达式，decltype能够告诉你该名字或者表达式的类型
