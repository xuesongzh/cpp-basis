...
template <typename T>
void myFunction(T &&tem) {  // T是类型模板参数，T是有类型的,tem是形参，tem也是有类型的
...
int main(void) {
    ...
    myFunction(i);  // 实例化后，我们认为编译器推断后的结构为void myFunction(int& &&tem){}
    myFunction(12);
    ...
    // int & &rr = b;//连接在一起叫右值引用，分开叫引用的引用，是非法的。但是编译器内部可以出现，它自己推导，使用引用折叠处理。
...
/*
 * 引用折叠规则
 * 引用折叠是c++11新标准，是一条规则，是一条规则reference-collapsing rules 引用坍塌，&&&--->&
