...
class AT {
 public:
    ...
    void addItem() {
         ...
         *但是如果我们在调用lambda之前删除对象的指针，结果是：
         *m_temvalue=-572662307
         *0
         *也就是lambda执行的正确与否取决于pat对象是否存在，这个lambda表达式在对象存在的时候才能正确执行。
         */
    }
};
