...
//模板函数，要把收到的参数以及这些参数相对应的类型保持不变转发为其他函数myFunction  --这就是转发
...
void myFunctionTramsmit02(F f, T1 &&t1, T2 &&t2) {
     ...
     */
    // f(t1, t2);
    //“void(int &&, int &)”: 无法将参数1 从“int”转换为“int &&”
...
int main(void) {
    ...
    /*
     * number01的类型是右值引用，但是本身是个左值，在内存中有地址。也就是说左值引用，右值引用概念说的是它的类型。
     * void myFunctionTramsmit02(F f, T1 &&t1, T2 &&t2)函数形参中的变量都是左值，本身类型是右值引用
     */

    myFunction03(number01, i);  //error，形参中是一个右值引用，必须绑定一个右值，number01本质上是一个左值
...
* (4)总结
*   forward()是强制把一个左值转换为右值，如果你的原始形参就是一个左值，那我的forward啥也不干
