...
int main(void) {
    //	2.1 new对象的时候加不加()的差别
    //  如果是个空类，两个写法没有区别。
    //  如果类中有成员变量，带()初始化会把一些和成员变量有关的内存清0，但是不是整个内存全部清0；

    //  2.2 当类有构造函数，两种写法一样。当没有构造函数，但是有一个虚函数的时候，还是会把成员变量m_i清0
  ...
  总结：new 调用了operator new函数，在 operator new函数里面调用malloc()，然后调用构造函数构造对象
       delete 的时候先调用析构函数，然后调用operator delete函数，在 operator delete函数里面调用free()释放内存。
   */
