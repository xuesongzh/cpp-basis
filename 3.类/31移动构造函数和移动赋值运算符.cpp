...
int main(void) {
    ...
    //如何将a1内容移动给a3？-->使用std::move()转换为一个右值，就会调用移动构造函数
...
/*
*(1)对象移动概念
*	进行对象的拷贝系统性能消耗太大。所以c++中引入“对象移动”的概念。
*   将一个不用的对象的内存移动给其他对象，这样可以节约系统性能。
*
*(2)移动构造函数和移动赋值运算符概念（c++11新引入的）
*程序执行的效率问题。
*移动构造函数：c++引入，进一步提高程序运行效率。
*	1.如果移动数据，A移动到B，那么A对象我们就不能再使用了。
...
*	2.确保移动后原对象处于一种“即便被销毁也没什么问题”的一种状态。确保不再使用A。
*	
*
*(3)移动赋值运算符--使用&&
*
...
*
*(4)合成的移动操作
*	    1.如果不写拷贝构造函数、拷贝赋值运算符，某些条件下可以自己合成移动构造函数，移动赋值运算符
*		2.有自己的拷贝构造函数，自己的拷贝赋值运算符，或者自己的析构，那么编译器就不会合成移动构造函数和移动赋值运算符。
*			所以一些类是没有合成移动构造函数和移动赋值运算符。
*			说明移动构造函数和移动赋值运算符是非常讲究的，不是那么容易实现的。
*		3.如果没有移动构造函数和移动赋值运算符，系统会调用拷贝构造函数和赋值运算符顶替，虽然参数不一样
*		4.只有一个类没有定义拷贝构造函数、拷贝赋值运算符、析构函数，且每个非静态成员都可以移动时，编译器才会为该类合成移动构造函数和移动赋值运算符。
*			什么叫成员可移动？
