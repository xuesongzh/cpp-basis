...
void playObject() {
    B1 *b = new C(1, 2, 3);  //父类指向子类对象
    //A *a = new C(1, 2, 3);//不能使用爷爷类
...
*	1.派生类可以通过它的两个直接基类分别继承同一个间接基类（爷爷类是一个）
...
*	  因为父类B1、B2不确定哪一个类进行初始化。相应的父类构造函数也不需要初始化爷爷类对象。
*	2.初始化顺序发生改变。先初始化虚基类部分A，然后按照派生列表的顺序来初始化其他类。
*	3.多个虚基类，哪个先初始化？
*		按照派生列表中虚基类出现的顺序构造
*总结
*	1.小心虚继承的二义性问题
