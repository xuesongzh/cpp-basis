...
int main(void) {
    ...
    Man *m2 = dynamic_cast<Man *>(p_human);  //父类指针转换为子类类型指针
	 ...
	 * 则typeid()返回的是静态类型（定义时候的类型），既然是定义的类型，编译器就不需要对表达式求值也知道表达式的类型。
	 *	
	 */
 ...
 *	主要方法：
 *		typeid().name()返回常量对象的名字
 *	主要用于：用于比较两个指针是否指向同一类型的对象，两个指针定义的类型相同
 *	
 *(3)RTTI与虚函数表
