...
int main(void) {
	 ...
	 *
	 * i++为什么是一个右值表达式？？？
	 * 先用后加，先产生一个临时变量，记录的值用于使用，再给i加1，接着返回这个临时变量（临时变量是右值）
	 */
...
*
*1.所有的变量，看成左值，因为他们是有地址的
*2.任何函数的形参都是左值。如void function(int &a);a虽然是引用，但是是左值
*3.临时对象是右值。
*
*
*(4)右值引入目的
*	1.右值引用是c++11的新概念，可以认为&&是一种新的数据类型，引入这种新的数据类型目的见2.
*	2.&&是为了提高运算效率，把拷贝对象变成移动对象来提高效率。
*     拷贝时候需要分配内存，然后再一个一个项拷贝，移动直接把内存块指针转给B，而不需要分配内存。具体见移动构造函数章节。
*	3.移动对象是如何发生的？怎么触发移动对象？
*		当用一个对象给另一个对象赋值，移动构造函数，移动赋值运算符（需要的参数是右值引用）
